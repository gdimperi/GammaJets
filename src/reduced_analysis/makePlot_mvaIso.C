...
void makePlot_mvaIso(int isEB, int isSig, int hlt){
  ...
  if(isEB){
    if (isSig) {
      sprintf(filename, "histo_v6/genIso4/mvaIso_hlt%d_histo_EB_isMatch.root", hlt);
      inputFile = new TFile(filename, "READ");
    }
    else if(!isSig){
      sprintf(filename, "histo_v6/genIso4/mvaIso_hlt%d_histo_EB_isNotMatch.root", hlt);
      inputFile = new TFile(filename, "READ");
    }
  }
  else if(!isEB) {
    if (isSig) {
      sprintf(filename, "histo_v6/genIso4/mvaIso_hlt%d_histo_EE_isMatch.root", hlt);
      inputFile = new TFile(filename, "READ");
    }
    else if(!isSig) {
      sprintf(filename, "histo_v6/genIso4/mvaIso_hlt%d_histo_EE_isNotMatch.root", hlt);
      inputFile = new TFile(filename, "READ");
    }
  }
  ...
  correl_plot_isoCharged ->GetYaxis()->SetTitle("PF iso Charged 03  [GeV]");
  correl_plot_isoNeutrals->GetYaxis()->SetTitle("PF iso Neutrals 03  [GeV]");
  correl_plot_isoPhotons ->GetYaxis()->SetTitle("PF iso Photons 03  [GeV]");
  ...
  correl_plot_isoFPRCharged ->GetYaxis()->SetTitle("PF iso Charged 03 with FPR [GeV]");
  correl_plot_isoFPRNeutrals->GetYaxis()->SetTitle("PF iso Neutrals 03 with FPR [GeV]");
  correl_plot_isoFPRPhotons ->GetYaxis()->SetTitle("PF iso Photons 03 with FPR [GeV]");
  ...
  BDToutput->GetXaxis()->SetTitle("BDT output");
  ...
  else if(hlt==30)   outputDir = "histo_v6/genIso4/HLT30/";
