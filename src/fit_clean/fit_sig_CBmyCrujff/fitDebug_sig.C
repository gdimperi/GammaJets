...
void fitDebug_sig(string cut, string filename, string hlt, bool binned){
  ...
  RooFitResult* result;
  //unbinned
  if(!binned) result =  CBmyCrujff_s.fitTo(*d_s, Save(), Range(-5.,15.), SumW2Error(kFALSE));
  if(binned)  result =  CBmyCrujff_s.fitTo(dh_s, Save(), Range(-5.,15.), SumW2Error(kFALSE));
  ...
  CBmyCrujff_s.plotOn(frame_s,Name("pdf_s"),LineColor(kCyan)) ;
