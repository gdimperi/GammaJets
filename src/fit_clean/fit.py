...
def main(options,args):
    ...
    print "--isoWeight   0 for central value, 1,2 for syst. uncert. studies"
...
#++++++++++++++++++++++++++++++++++++
    ...
    #ptBins_EB=[]
    #ptBins_EE=[]
    ptBins_EB=[bin_EB[options.bin]]
    ptBins_EE=[bin_EE[options.bin]]
    ...
    #gROOT.ProcessLine(".L fitDebug_sigNonParam.C++")
    #gROOT.ProcessLine(".L fitDebug_weightedDataCS.C++")
    #gROOT.ProcessLine(".L fitDebug_sig.C++")
    #gROOT.ProcessLine(".L fitDebug_nonParam.C++")
    gROOT.ProcessLine(".L fitDebug_sigZeeNonParam.C+")
    #gROOT.ProcessLine(".L fitDebug_sigZeeParam.C++")

#------------------------------
#  For signal we use data CS of electrons in 40<pt<65 GeV pt region for all bins
#----------------------------
    ...
    if options.isEB:
        ...
        while i < len(ptBins_EB):
            ...
            #fitDebug('abs(etaPhot)<1.4442  && ptPhot>'+ptBins_EB[i]['ptMin']+'. && ptPhot<'+ptBins_EB[i]["ptMax"]+'.', 'fit_EB_bg_WP095_pt'+ptBins_EB[i]['ptMin']+'_'+ptBins_EB[i]['ptMax'], ptBins_EB[i]['hlt'], 0, options.outdir, options.isoWeight, options.isFPR, options.isEB)
            fitDebug_sigNonParam('', 'fit_EB_sig_WP095_pt'+ptBins_EB[i]['ptMin']+'_'+ptBins_EB[i]['ptMax'], ptBins_EB[i]['hlt'], options.outdir, options.isFPR, options.isEB)
    ...
    #loop over EE bins
        ...
        while i < len(ptBins_EE):
            ...
            #no condition on isMatched/isIsolated because fitting on data CS
            #fitDebug('abs(etaPhot)>1.556 && abs(etaPhot)<2.5  && ptPhot>'+ptBins_EE[i]['ptMin']+'. && ptPhot<'+ptBins_EE[i]["ptMax"]+'.', 'fit_EE_bg_WP095_pt'+ptBins_EE[i]['ptMin']+'_'+ptBins_EE[i]['ptMax'], ptBins_EE[i]['hlt'], 0, options.outdir, options.isoWeight, options.isFPR, options.isEB)
            fitDebug_sigNonParam('', 'fit_EE_sig_WP095_pt'+ptBins_EE[i]['ptMin']+'_'+ptBins_EE[i]['ptMax'], ptBins_EE[i]['hlt'], options.outdir,  options.isFPR, options.isEB)
...
if __name__ == "__main__":
    parser = OptionParser(option_list=[
        ...
        make_option("--bin",
                    action="store", type="int", dest="bin",
                    default=0,
                    help="", metavar=""
                    ),
        ])
