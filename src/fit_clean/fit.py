...
#++++++++++++++++++++++++++++++++++++
    ...
    if options.isEB:
        ...
        while i < len(ptBins_EB):
            ...
            fitDebug('abs(etaPhot)<1.4442  && ptPhot>'+ptBins[i]['ptMin']+'. && ptPhot<'+ptBins_EB[i]["ptMax"]+'.', 'fit_EB_bg_WP095_pt'+ptBins_EB[i]['ptMin']+'_'+ptBins_EB[i]['ptMax'], ptBins_EB[i]['hlt'], 0, options.outdir, options.isoWeight, options.isFPR, options.isEB)
            #fitDebug_sig('abs(etaPhot)<1.4442  && isIsolatedGenPhot==1 && ptPhot>'+ptBins_EB[i]['ptMin']+'. && ptPhot<'+ptBins_EB[i]['ptMax']+'.', 'fit_EB_sig_WP095_pt'+ptBins_EB[i]['ptMin']+'_'+ptBins_EB[i]['ptMax'], ptBins_EB[i]['hlt'], 0, options.outdir, options.isFPR, options.isEB)
            i=i+1
    ...
    #loop over EE bins
        ...
        while i < len(ptBins_EE):
            #no condition on isMatched/isIsolated because fitting on data CS
            fitDebug('abs(etaPhot)>1.556 && abs(etaPhot)<2.5  && ptPhot>'+ptBins_EE[i]['ptMin']+'. && ptPhot<'+ptBins_EE[i]["ptMax"]+'.', 'fit_EB_bg_WP095_pt'+ptBins_EE[i]['ptMin']+'_'+ptBins_EE[i]['ptMax'], ptBins_EE[i]['hlt'], 0, options.outdir, options.isoWeight, options.isFPR, options.isEB)
            #fitDebug_sig('abs(etaPhot)>1.556 && abs(etaPhot)<2.5  && isIsolatedGenPhot==1 && ptPhot>'+ptBins_EE[i]['ptMin']+'. && ptPhot<'+ptBins_EE[i]['ptMax']+'.', 'fit_EE_sig_WP095_pt'+ptBins_EE[i]['ptMin']+'_'+ptBins_EE[i]['ptMax'], ptBins_EE[i]['hlt'], 0, options.outdir, options.isFPR, options.isEB)
            i=i+1
