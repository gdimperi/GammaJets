...
bin[12]=dict(hlt='hltcut90', ptMin='139', ptMax='163')
bin[13]=dict(hlt='hltcut90', ptMin='163', ptMax='1000')
...
while i < len(ptBins):
    ...
    fitDebug('abs(etaPhot)<1.4442  && isMatchedPhot==0 && ptPhot>'+ptBins[i]['ptMin']+'. && ptPhot<'+ptBins[i]["ptMax"]+'.', 'fit_EB_bg_WP095_pt'+ptBins[i]['ptMin']+'_'+ptBins[i]['ptMax'], ptBins[i]['hlt'], 0)
    fitDebug_sig('abs(etaPhot)<1.4442  && isMatchedPhot==1 && ptPhot>'+ptBins[i]['ptMin']+'. && ptPhot<'+ptBins[i]['ptMax']+'.', 'fit_EB_sig_WP095_pt'+ptBins[i]['ptMin']+'_'+ptBins[i]['ptMax'], ptBins[i]['hlt'], 0)
    i=i+1
