...
def main(options,args):
    ...
    #gROOT.ProcessLine(".L plotPull_nonParam.C+")
    #gROOT.ProcessLine(".L fitData_myPull_nonParam.C+")
    
    
    i = 0
    while i < len(ptBins):
        print ptBins[i]['ptMin']+' '+ptBins[i]['ptMax']
        #fitData_myPull(ptBins[i]['ptMin'], ptBins[i]['ptMax'], ptBins[i]['nev'], ptBins[i]['pur'], options.config, options.inputdir, options.outdir, options.isFPR)
        fitData_myPull(ptBins[i]['ptMin'], ptBins[i]['ptMax'], ptBins[i]['nev'], ptBins[i]['pur'], options.config, options.inputdir, options.outdir)
        plotPull(ptBins[i]['ptMin'], ptBins[i]['ptMax'], options.outdir)
        i=i+1
...
if __name__ == "__main__":
    parser = OptionParser(option_list=[
        ...
        make_option("--inputdir",
                    ...
                    #default="/cmshome/gdimperi/GammaJet/CMSSW_6_0_1/src/GammaJets/src/fit_clean/Pull/",
                    help="", metavar=""
                    ),
        make_option("--config",
