...
def composeDirPaths(pars):
    #directories for different hlts containing plots to be merged
    dirPlotToMerge={}
    dirPlotToMerge['hlt30']='plots_hlt30_mvaWP'+str(pars['mvaWP'])+str(pars['selType'])
    dirPlotToMerge['hlt50']='plots_hlt50_mvaWP'+str(pars['mvaWP'])+str(pars['selType'])
    dirPlotToMerge['hlt75']='plots_hlt75_mvaWP'+str(pars['mvaWP'])+str(pars['selType'])
    dirPlotToMerge['hlt90']='plots_hlt90_mvaWP'+str(pars['mvaWP'])+str(pars['selType'])
    return dirPlotToMerge
...
def plot(pars,savefmts=[".C",".png",".pdf",".root"]):
    dirPlotToMerge=composeDirPaths(pars)
    #customizeCanvas(c)
    ...
    for cut in cuts.keys():
        for hltDir in dirPlotToMerge:
            f = TFile(str(hltDir)+'/'+str(pars['variable'])+str(cut)+".root")
            c = f.Get(str(pars['variable']))
            temp = c.GetPrimitive(str(plotPars[str(pars['variable'])]['name'])+"_data_"+str(cut)).Clone()
            histos[str(pars['variable'])+'_data_'+str(cut)] = TH1F()
            histos[str(pars['variable'])+'_data_'+str(cut)].Sumw2()
            histos[str(pars['variable'])+'_data_'+str(cut)].Add(temp)
            for t in mc_types.keys():
                temp = c.GetPrimitive(str(plotPars[str(pars['variable'])]['name'])+"_mc_"+t+"_"+str(cut)).Clone()
                histos[str(pars['variable'])+'_mc_'+t+"_"+str(cut)]=TH1F()
                histos[str(pars['variable'])+'_mc_'+t+"_"+str(cut)].Sumw2()
                histos[str(pars['variable'])+'_mc_'+t+"_"+str(cut)].Add(temp)  
...
def main(options,args):
    ...
    MergedPlotsDir=options.MergedPlotsDir
    ...
    pars=[ dict(variable=x,mvaWP=mvaWP,selType=selType,additional_cuts=additionalCuts,additional_text=additionalText) for x in variables.keys() ]
    #      print pars
    pool.map(plot, pars)
...
if __name__ == "__main__":
    parser = OptionParser(option_list=[
        make_option("--mvaWP",
                    action="store", type="string", dest="mvaWP",
                    default="0",
                    help="", metavar=""
                    ),
        make_option("--selType",
                    action="store", type="string", dest="selType",
                    default="_ssel",
                    help="", metavar=""
                    ),
        make_option("--MergedPlotsDir",
                    action="store", type="string", dest="MergedPlotsDir",
                    default="plots/",
                    help="", metavar=""
                    ),
